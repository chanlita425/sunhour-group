...
@section('content')
    <div class="w-full mx-auto">
        <nav class="grid grid-cols-12">
            <form action="{{ route('daily-cleans.index',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="GET" class="col-span-8 w-[54vh]">
        ...
        <div class="mt-5">
            <div class="grid grid-cols-12">
                ...
                <div class="col-span-8 flex items-center justify-end space-x-5">
                    <form action="{{ route('daily-cleans.index',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="GET" id="perPage">
                        <label for="perPage">
                            Showing
    ...
    <div class="grid grid-cols-12 space-x-5 mt-5">
                        ...
                        @if($item->model_id == $singleData->uuid)
                            ...
                            <td class="inline-flex items-center gap-2 float-end">
                                <button data-tip="Edit"
                                        data-id="{{ $item->uuid }}"
                                        data-url="{{ route('daily-cleans.edit', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                        data-update-url="{{ route('daily-cleans.update', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                ...
                                <button data-id="{{ $item->uuid }}"
                                        data-url="{{ route('daily-cleans.destroy', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                        data-tip="Delete"
        ...
        <div id="formAdd" class="col-span-4 xl:col-span-3 w-full pr-5">
             <form action="{{ route('daily-cleans.store', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="POST" class="w-full bg-white rounded-lg p-5" id="brandFormAdd">
                @csrf
                <div x-data="{
                    name: '',
                    video: '',
    ...
    <script>
        ...
        document.getElementById('brandFormAdd').addEventListener('submit', function(e) {
            ...
            fetch("{{ route('daily-cleans.store',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}", {
                method: 'POST',
                body: formData,
                headers: {
