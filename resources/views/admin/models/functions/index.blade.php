...
@section('content')
    <div class="w-full mx-auto">
        <nav class="grid grid-cols-12">
            <form action="{{ route('functions.index',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="GET" class="col-span-8 w-[54vh]">
        ...
        <div class="mt-5">
            <div class="grid grid-cols-12">
                ...
                <div class="col-span-8 flex items-center justify-end space-x-5">
                    <!-- Per Page Form -->
                    <form action="{{ route('functions.index',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="GET" id="perPage">
                        <label for="perPage">
                            Showing
    ...
    <div class="grid grid-cols-12 space-x-5 mt-5">
        <div class="bg-white rounded-lg col-span-8 xl:col-span-9 h-[75vh] xl:h-[85vh] overflow-auto">
                                ...
                                <td class="inline-flex items-center gap-2 float-end">
                                    <div class="dropdown dropdown-hover dropdown-end">
                                        ...
                                        <ul tabindex="0"
                                            ...
                                            <li>
                                                <a href="{{ route('tech-details.index',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                    ...
                                    <button data-tip="Edit"
                                            data-id="{{ $item->uuid }}"
                                            data-url="{{ route('functions.edit', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                            data-update-url="{{ route('functions.update', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                    ...
                                    <button data-id="{{ $item->uuid }}"
                                            data-url="{{ route('functions.destroy', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                            data-tip="Delete"
            ...
            <div class="m-3">
                {{ $functions->links('pagination::daisyui') }}
            </div>
        </div>

        {{-- form to add new brand --}}
        <div id="formAdd" class="col-span-4 xl:col-span-3 w-full pr-5">
             <form action="{{ route('functions.store', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="POST" class="w-full bg-white rounded-lg p-5" id="brandFormAdd">
                @csrf
                <div x-data="{
                    name: '',
                    icon: '',
                ...
                class="space-y-4">
                    ...
                    <div class="form-group w-full space-y-2">
                        ...
                        <label class="inline-flex items-center gap-2 w-full bg-gray-100 rounded-sm py-1 px-2 text-[12px] font-light outline-none focus:bg-gray-200 transition-all duration-300">
                            ...
                            <input type="url" name="icon" x-model="icon"
    ...
    <script>
        ...
        document.getElementById('brandFormAdd').addEventListener('submit', function(e) {
            ...
            fetch("{{ route('functions.store',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}", {
                method: 'POST',
                body: formData,
                headers: {
