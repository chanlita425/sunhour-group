...
@section('content')
    <div class="w-full mx-auto">
        <nav class="grid grid-cols-12">
            <form action="{{ route('tech-details.index',[$singleData->models->products->brands->uuid,$singleData->models->products->uuid,$singleData->models->uuid, $singleData->uuid]) }}" method="GET" class="col-span-8 w-[54vh]">
        ...
        <div class="mt-5">
            <div class="grid grid-cols-12">
                <div class="breadcrumbs text-sm col-span-4">
                    <ul>
                        <li>
                            <a href="{{ route('brands.index', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid]) }}" class="flex items-center gap-2">
                        ...
                        <li>
                            <a href="{{ route('products.index', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid]) }}">Products</a>
                        </li>
                        <li>
                            <a href="{{ route('models.index', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid]) }}">Models</a>
                        </li>
                        <li>
                            <a href="{{ route('functions.index', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid, $singleData->models->uuid]) }}">Functions</a>
                        </li>
                        <li>Add Details</li>
                    </ul>
                </div>
                <div class="col-span-8 flex items-center justify-end space-x-5">
                    <!-- Per Page Form -->
                    <form action="{{ route('tech-details.index',[$singleData->models->products->brands->uuid,$singleData->models->products->uuid,$singleData->models->uuid, $singleData->uuid]) }}" method="GET" id="perPage">
                        <label for="perPage">
                            Showing
    ...
    <div class="grid grid-cols-12 space-x-5 mt-5">
        <div class="bg-white rounded-lg col-span-8 xl:col-span-9 h-[75vh] xl:h-[85vh] overflow-auto">
                ...
                @else
                    <tbody>
                        @foreach($tech as $item)
                            @if($item->functions_id === $singleData->uuid)
                                <tr class="w-full hover:bg-gray-100 border-gray-200 border-b">
                                    ...
                                    <td >
                                        <div class="flex items-center gap-2">
                                            <iframe width="200" height="100" src="{{$item->link}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                                            <p class="capitalize">
                                                {{$item->name}}
                                            </p>
                                        </div>
                                    ...
                                    <td class="overflow-hidden">
                                        <p class="capitalize text-balance line-clamp-2">{{$item->description}} </p>
                                    </td>
                                    <td class="inline-flex items-center gap-2 float-end">
                                        <button data-tip="Edit"
                                                data-id="{{ $item->uuid }}"
                                                data-url="{{ route('tech-details.edit', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid, $singleData->models->uuid,$singleData->uuid,$item->uuid]) }}"
                                                data-update-url="{{ route('tech-details.update', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid, $singleData->models->uuid,$singleData->uuid,$item->uuid]) }}"
                                        ...
                                        <button data-id="{{ $item->uuid }}"
                                                data-url="{{ route('tech-details.destroy', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid, $singleData->models->uuid,$singleData->uuid,$item->uuid]) }}"
                                                data-tip="Delete"
            ...
            <div class="m-3">
                {{ $tech->links('pagination::daisyui') }}
            </div>
        </div>

        {{-- form to add new brand --}}
        <div id="formAdd" class="col-span-4 xl:col-span-3 w-full pr-5">
             <form action="{{ route('tech-details.store', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid, $singleData->models->uuid,$singleData->uuid]) }}" method="POST" class="w-full bg-white rounded-lg p-5" id="brandFormAdd">
                @csrf
                <div x-data="{
                    link: '',
                    isFormValid() {
                ...
                class="space-y-4">
                    <input type="hidden" name="functions_id" value="{{ $singleData->uuid }}"
                    ...
                    <div class="form-group w-full space-y-2">
                        ...
                        <input type="text" readonly name="name" x-text="{{$singleData->name}}" value="{{$singleData->name}}"
                            class="form-control w-full bg-gray-100 rounded-sm py-1 px-2 text-[12px] font-light outline-none focus:bg-gray-200 transition-all duration-300"
                            placeholder="Enter Daily Clean">
                    </div>
                    <!-- Link Input -->
        ...
        <div id="formEdit" class="hidden col-span-4 xl:col-span-3 w-full pr-5">
            <form method="POST" class="w-full bg-white rounded-lg p-5" id="brandFormEdit">
                ...
                <div
                     class="space-y-4">
                    <input type="hidden" name="functions_id" id="functions_id"
                           class="form-control w-full bg-gray-100 rounded-sm py-1 px-2 text-[12px] font-light outline-none focus:bg-gray-200 transition-all duration-300"
                           placeholder="Enter Daily Clean">
                    <!-- Daily Clean Input -->
                    <div class="form-group w-full space-y-2">
    ...
    <script>
        ...
        document.getElementById('brandFormAdd').addEventListener('submit', function(e) {
            ...
            fetch("{{ route('tech-details.store', [$singleData->models->products->brands->uuid, $singleData->models->products->uuid, $singleData->models->uuid,$singleData->uuid]) }}", {
                method: 'POST',
                body: formData,
                headers: {
            ...
            button.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('functions_id').value = data.data.functions_id || '';
