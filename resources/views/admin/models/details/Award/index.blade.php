...
@section('content')
    <div class="w-full mx-auto">
        <nav class="grid grid-cols-12">
            <form
                action="{{ route('awards.index', [$singleData->products->brands->uuid, $singleData->products->uuid, $singleData->uuid]) }}"
                method="GET" class="col-span-8 w-[54vh]">
        ...
        <div class="mt-5">
            <div class="grid grid-cols-12">
                <div class="breadcrumbs text-sm col-span-4">
                    <ul>
                        <li>
                            <a href="{{ route('brands.index', [$singleData->products->brands->uuid]) }}"
                        ...
                        <li>
                            <a
                                href="{{ route('products.index', [$singleData->products->brands->uuid, $singleData->products->uuid]) }}">Products</a>
                        </li>
                        <li>
                            <a
                                href="{{ route('models.index', [$singleData->products->brands->uuid, $singleData->products->uuid]) }}">Models</a>
                        </li>
                        <li>Add Award</li>
                    </ul>
                </div>
    ...
    <div class="grid grid-cols-12 space-x-5 mt-5">
        <div class="bg-white rounded-lg col-span-8 xl:col-span-9 h-[75vh] xl:h-[85vh] overflow-auto">
                                ...
                                <tr class="w-full hover:bg-gray-100 border-gray-200 border-b">
                                    ...
                                    <td>
                                        <div class="inline-flex items-center gap-2">
                                            <img src="{{ asset($item->path) }}" alt="" class="w-[6rem] h-[3rem]">
                                            {{-- <p>{{$item->name}}</p> --}}
                                        </div>
                                    </td>
                                    <td class="inline-flex items-center gap-2 float-end">
                                        <button data-tip="Edit" data-id="{{ $item->uuid }}"
                                            data-url="{{ route('awards.edit', [$singleData->products->brands->uuid, $singleData->products->uuid, $singleData->uuid, $item->uuid]) }}"
                                            data-update-url="{{ route('awards.update', [$singleData->products->brands->uuid, $singleData->products->uuid, $singleData->uuid, $item->uuid]) }}"
                                        ...
                                        <button data-id="{{ $item->uuid }}"
                                            data-url="{{ route('awards.destroy', [$singleData->products->brands->uuid, $singleData->products->uuid, $singleData->uuid, $item->uuid]) }}"
                                            data-tip="Delete"
            ...
            <div class="my-16">
                <div class="flex items-center justify-end">
                    {{ $award->links('pagination::daisyui') }}
                </div>
            </div>
        </div>
        ...
        <div id="formAdd" class="col-span-4 xl:col-span-3 w-full pr-5">
            <form
                action="{{ route('awards.store', [$singleData->products->brands->uuid, $singleData->products->uuid, $singleData->uuid]) }}"
                method="POST" class="w-full bg-white rounded-lg p-5" id="brandFormAdd">
                @csrf
                <div x-data="{
                    path: '',
                    isFormValid() {
                        return this.path.trim() !== '' && this.path.startsWith('http');
                    }
                }" class="space-y-4">
                    ...
                    <div class="form-group w-full space-y-2">
                        <label for="path" class="text-gray-500 text-[12px]">Link</label>
                        <label
                            class="inline-flex items-center gap-2 w-full bg-gray-100 rounded-sm py-1 px-2 text-[12px] font-light outline-none focus:bg-gray-200 transition-all duration-300">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" width="24"
                                height="24" stroke-width="1.25">
                            ...
                            <input type="url" name="path" x-model="path"
                                class="w-full outline-none bg-transparent" placeholder="https://placehold.co/100x100">
                        </label>
                    </div>
                    <!-- Submit Button -->
                ...
                <div class="space-y-4">
                    ...
                    <div class="form-group w-full space-y-2">
                        ...
                        <label
                            ...
                            <input type="url" name="path" id="path"
                                class="w-full outline-none bg-transparent" placeholder="https://placehold.co/100x100">
                        </label>
                    </div>
                    <!-- Submit Button -->
    ...
    <script>
        ...
        document.getElementById('brandFormAdd').addEventListener('submit', function(e) {
            ...
            fetch("{{ route('awards.store', [$singleData->products->brands->uuid, $singleData->products->uuid, $singleData->uuid]) }}", {
                    method: 'POST',
                    body: formData,
                    headers: {
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', async function() {
                ...
                const updateUrl = this.getAttribute(
                'data-update-url'); // e.g., /admin/brands/update/{uuid}
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('path').value = data.data.path || '';
