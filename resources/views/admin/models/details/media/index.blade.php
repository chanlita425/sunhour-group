...
@section('content')
    <div class="w-full mx-auto">
        <nav class="grid grid-cols-12">
            <form action="{{ route('medias.index',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="GET" class="col-span-8 w-[54vh]">
        ...
        <div class="mt-5">
            <div class="grid grid-cols-12">
                <div class="breadcrumbs text-sm col-span-4">
                    <ul>
                        <li>
                            <a href="{{ route('brands.index', [$singleData->products->brands->uuid]) }}" class="flex items-center gap-2">
                ...
                <div class="col-span-8 flex items-center justify-end space-x-5">
                    <form action="{{ route('medias.index',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="GET" id="perPage">
                        <label for="perPage">
                            Showing
    ...
    <div class="grid grid-cols-12 space-x-5 mt-5">
        <div class="bg-white rounded-lg col-span-8 xl:col-span-9 h-[75vh] xl:h-[85vh] overflow-auto">
              @if ($loading)
                    <div class="inline-flex items-end gap-2 p-5">
                        <p>Loading</p>
                        <span class="loading loading-dots loading-xs text-gray-500"></span>
                    </div>
                @else
                    <div class="w-full p-5 flex flex-wrap items-center gap-x-5 gap-y-10">
                        @foreach($media as $item)
                            @if($item->model_id === $singleData->uuid)
                                <article class="w-full mx-auto flex flex-col items-start gap-2">
                                    <div class="flex items-center justify-between w-full">
                                        <p>{{$item->name}}</p>
                                        <div class="inline-flex items-center gap-2 float-end">
                                            <button data-tip="Edit"
                                                    data-id="{{ $item->uuid }}"
                                                    data-url="{{ route('medias.edit', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                                    data-update-url="{{ route('medias.update', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                            ...
                                            <button data-id="{{ $item->uuid }}"
                                                    data-url="{{ route('medias.destroy', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid,$item->uuid]) }}"
                                                    data-tip="Delete"
                                    ...
                                    <iframe width="1050" height="720" src="{{$item->link}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                                </article>
                            @endif
                        @endforeach
                    </div>
            ...
            <div class="my-16">
                <div class="flex items-center justify-end">
                    {{ $media->links('pagination::daisyui') }}
                </div>
            </div>
        </div>
        ...
        <div id="formAdd" class="col-span-4 xl:col-span-3 w-full pr-5">
             <form action="{{ route('medias.store', [$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}" method="POST" class="w-full bg-white rounded-lg p-5" id="brandFormAdd">
                @csrf
                <div x-data="{
                    name: '',
                    link: '',
    ...
    <script>
        ...
        document.getElementById('brandFormAdd').addEventListener('submit', function(e) {
            ...
            fetch("{{ route('medias.store',[$singleData->products->brands->uuid,$singleData->products->uuid,$singleData->uuid]) }}", {
                method: 'POST',
                body: formData,
                headers: {
